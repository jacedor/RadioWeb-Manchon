SET TERM ^ ;
ALTER PROCEDURE FACTURACION_MENSUAL (
...
BEGIN
 ...
 for select 1 AS ORDEN,G.COD_GRUP,EXTRACT(MONTH FROM E.FECHA) AS MES,
    ...
    group by G.COD_GRUP,EXTRACT(MONTH FROM E.FECHA)
    ...
    select 2 AS ORDEN,'TOTAL' AS COD_GRUP, EXTRACT(MONTH FROM E.FECHA) AS  MES,
        SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, e.cantidad, 0)) as sAnterior,
        AVG(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, e.cantidad, 0)) as PAnterior,
        SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, e.cantidad, 0)) as sActual,
        AVG(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, e.cantidad, 0)) as PActual,
    ...
    and (e.IOR_GPR BETWEEN :pGPRMin and :pGPRMax) and (e.PAGADO LIKE '%'||:pPagado) and (e.FACTURADA LIKE '%'||:pFACTURADO)
    group by COD_GRUP, EXTRACT(MONTH FROM E.FECHA) 
    --PLAN JOIN (SORT (JOIN ( E INDEX (IDXEXPLORACIONFECHA,IDXEXPLORACIONCOLEGIADO,IDXEXPLORACIONMEDICO, IDXEXPLORACIONMUTUAS))))  
 

  INTO :ORDEN,:COD_GRUP,:MES,:SANTERIOR,:PANTERIOR,:SACTUAL,:PACTUAL,:CANTERIOR,:CACTUAL,:DIFCUENTA,:DIFCUENTAPORC
 
  do begin
    INSERT INTO HFACTURACIONMENSUAL (ORDEN,COD_GRUP,MES,SANTERIOR,PANTERIOR,SACTUAL,PACTUAL,CANTERIOR,CACTUAL,DIFCUENTA,DIFCUENTAPORC)
       VALUES (:ORDEN,:COD_GRUP,:MES,:SANTERIOR,:PANTERIOR,:SACTUAL,:PACTUAL,:CANTERIOR,:CACTUAL,:DIFCUENTA,:DIFCUENTAPORC);   
  end


  FOR SELECT G.COD_GRUP, EXTRACT(MONTH FROM E.FECHA) AS  MES,
    ...
    SELECT 'TOTAL' AS COD_GRUP, EXTRACT(MONTH FROM E.FECHA) AS  MES,
            SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, x.PRECIO, 0)) as sAnteriorConsum,
            SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, x.PRECIO, 0)) as sActualConsum  
    from EXP_CONSUM X
    JOIN EXPLORACION E ON E.OID=X.IOR_EXPLORACION 
  ...
  INTO :COD_GRUP,:MES,:SANTERIORCONSUM,SACTUALCONSUM
  do begin 
    UPDATE HFACTURACIONMENSUAL SET SANTERIORCONSUM=:SANTERIORCONSUM, SACTUALCONSUM=:SACTUALCONSUM
        WHERE MES=:MES AND COD_GRUP=:COD_GRUP;
  end

  FOR SELECT ORDEN,COD_GRUP,MES,SANTERIOR,PANTERIOR,SANTERIORCONSUM,
    ...
    ORDER BY 3,1,2
    INTO :ORDEN,:COD_GRUP,:MES,:SANTERIOR,:PANTERIOR,:SANTERIORCONSUM,:SANTERIORTOTAL,:SACTUAL,:PACTUAL,:SACTUALCONSUM,:SACTUALTOTAL,:DIFCANTIDAD,:DIFCANTIDADPORC,:CANTERIOR,:CACTUAL,:DIFCUENTA,:DIFCUENTAPORC
...
GRANT EXECUTE
 ON PROCEDURE FACTURACION_MENSUAL TO  AFFIDEA;

GRANT EXECUTE
 ON PROCEDURE FACTURACION_MENSUAL TO  SYSDBA;
