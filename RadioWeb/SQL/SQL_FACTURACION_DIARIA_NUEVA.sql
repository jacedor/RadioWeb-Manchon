SET TERM ^ ;
ALTER PROCEDURE FACTURACION_DIARIA (
...
BEGIN
 ...
 for select 1 AS ORDEN,G.COD_GRUP,EXTRACT(MONTH FROM E.FECHA) AS MES,EXTRACT(DAY FROM E.FECHA) AS DIA,
        SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, e.cantidad, 0)) as sAnterior,
        AVG(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, e.cantidad, 0)) as PAnterior,
        SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, e.cantidad, 0)) as sActual,
        AVG(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, e.cantidad, 0)) as PActual,
    ...
    and (e.IOR_GPR BETWEEN :pGPRMin and :pGPRMax) and (e.PAGADO LIKE '%'||:pPagado)
    group by G.COD_GRUP,EXTRACT(MONTH FROM E.FECHA),EXTRACT(DAY FROM E.FECHA)
    ...
    select 2 AS ORDEN,'TOTAL' AS COD_GRUP,EXTRACT(MONTH FROM E.FECHA) AS MES,EXTRACT(DAY FROM E.FECHA) AS  DIA,
        SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, e.cantidad, 0)) as sAnterior,
        AVG(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, e.cantidad, 0)) as PAnterior,
        SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, e.cantidad, 0)) as sActual,
        AVG(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, e.cantidad, 0)) as PActual,
    ...
    and (e.IOR_GPR BETWEEN :pGPRMin and :pGPRMax) and (e.PAGADO LIKE '%'||:pPagado)
    group by COD_GRUP, EXTRACT(MONTH FROM E.FECHA),EXTRACT(DAY FROM E.FECHA) 
    --PLAN JOIN (SORT (JOIN ( E INDEX (IDXEXPLORACIONFECHA,IDXEXPLORACIONCOLEGIADO,IDXEXPLORACIONMEDICO, IDXEXPLORACIONMUTUAS))))  
 

  INTO :ORDEN,:COD_GRUP,:MES,:DIA,:SANTERIOR,:PANTERIOR,:SACTUAL,:PACTUAL,:CANTERIOR,:CACTUAL,:DIFCUENTA,:DIFCUENTAPORC
 
  do begin
    INSERT INTO HFACTURACIONMENSUAL (ORDEN,COD_GRUP,MES,DIA,SANTERIOR,PANTERIOR,SACTUAL,PACTUAL,CANTERIOR,CACTUAL,DIFCUENTA,DIFCUENTAPORC)
       VALUES (:ORDEN,:COD_GRUP,:MES,:DIA,:SANTERIOR,:PANTERIOR,:SACTUAL,:PACTUAL,:CANTERIOR,:CACTUAL,:DIFCUENTA,:DIFCUENTAPORC);   
  end


  FOR SELECT G.COD_GRUP, EXTRACT(MONTH FROM E.FECHA) AS MES,EXTRACT(DAY FROM E.FECHA) AS DIA ,
    ...
    SELECT 'TOTAL' AS COD_GRUP, EXTRACT(MONTH FROM E.FECHA) AS MES,EXTRACT(DAY FROM E.FECHA) AS  DIA,
            SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYPASSAT, x.PRECIO, 0)) as sAnteriorConsum,
            SUM(IIF(EXTRACT(YEAR FROM E.FECHA) = :ANYACTUAL, x.PRECIO, 0)) as sActualConsum  
    --from EXP_CONSUM X
   -- JOIN EXPLORACION E ON E.OID=X.IOR_EXPLORACION 
  ...
  INTO :COD_GRUP,:MES,:DIA,:SANTERIORCONSUM,SACTUALCONSUM
  do begin 
    UPDATE HFACTURACIONMENSUAL SET SANTERIORCONSUM=:SANTERIORCONSUM, SACTUALCONSUM=:SACTUALCONSUM
        WHERE MES=:MES AND DIA=:DIA AND COD_GRUP=:COD_GRUP;
  end

  FOR SELECT ORDEN,COD_GRUP,MES,DIA,SANTERIOR,PANTERIOR,SANTERIORCONSUM,
           (SANTERIOR+COALESCE(SANTERIORCONSUM,0)) AS SANTERIORTOTAL,
           SACTUAL,PACTUAL,SACTUALCONSUM,(SACTUAL+COALESCE(SACTUALCONSUM,0)) AS SACTUALTOTAL,
    ...
    ORDER BY 3,4,1,2
    INTO :ORDEN,:COD_GRUP,:MES,:DIA,:SANTERIOR,:PANTERIOR,:SANTERIORCONSUM,:SANTERIORTOTAL,:SACTUAL,:PACTUAL,:SACTUALCONSUM,:SACTUALTOTAL,:DIFCANTIDAD,:DIFCANTIDADPORC,:CANTERIOR,:CACTUAL,:DIFCUENTA,:DIFCUENTAPORC
...
GRANT EXECUTE
 ON PROCEDURE FACTURACION_DIARIA TO  AFFIDEA;

GRANT EXECUTE
 ON PROCEDURE FACTURACION_DIARIA TO  SYSDBA;
